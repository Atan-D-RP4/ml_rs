...
impl Network2 {
    ...
    fn backpropagation(
        ...
        let base_cost = self.cost(samples, targets)?;
        let mut gradients = Network2::new(self.w1.rows, self.w1.cols, self.w2.cols);

        if samples.rows != targets.rows {
            return Err(MatrixError::DimensionMismatch(format!(
                "Expected {} samples, got {}",
                samples.rows, targets.rows
            )));
        }
        if samples.cols != self.w1.rows {
            return Err(MatrixError::DimensionMismatch(format!(
                "Expected {} features, got {}",
                self.w1.rows, samples.cols
            )));
        }
        if targets.cols != self.w2.cols {
            return Err(MatrixError::DimensionMismatch(format!(
                "Expected {} targets, got {}",
                self.w2.cols, targets.cols
            )));
        }
